...
class GB_JSON_API {
  ...
  function admin_menu() {
    add_options_page('netGALAXY Studios Api Settings', 'netGALAXY Studios Api', 'manage_options', 'gb-json-api', array(&$this, 'admin_options'));
  }
  ...
  function admin_options() { 
	...
	<form action="options-general.php?page=gb-json-api" method="post" >
		...
		<div id='goodbarber' >
		<h1> netGALAXY Studios Api</h1>
		<?php _e('A JSON-API fork used to synchronize your netGALAXY Studios app with your wordpress.') ?><br/><br/>
		<h2>netGALAXY Studios Key </h2>
		<?php _e('Provide this key in your netGALAXY Studios back office when you first add this website as a source.') ?>
		<h1><?php echo get_option('gb_api_key', ''); ?></h1><br/>
	        <h2>Options</h2>
		<h3><?php _e('Comments submission')?> </h3>
		<?php $ice=get_option('is_comments_enabled', 'True');  ?>
		<?php _e('If you want to let your users post comments to your website right from your netGALAXY Studios mobile app, check the box below.') ?><br/><br/>
		...
		<h3><?php _e('Security management') ?> </h3>
                <?php _e('By default, netGALAXY Studios Api secure mode is off. If you want to limit the access to this plugin to netGALAXY Studios servers only, check the box below.') ?><br/><br/>
                <?php $isme= get_option('is_secure_mode_enabled', 'False'); ?>
                <input name='is_secure_mode_enabled' type="checkbox" <?php if($isme == 'True'){echo 'checked="checked"';} ?>><?php _e(' Enable secure mode (IP restriction to netGALAXY Studios servers only)') ?>
